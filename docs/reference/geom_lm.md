...
Adds linear model fits to plots. `geom_lm()` and `stat_lm()` are
essentially equivalent. Use `geom_lm()` unless you want a non-standard
geom.
...
- formula:
  ...
  a formula describing the model in terms of `y` (response) and `x`
  (predictor).

- lm.args:

  A list of arguments supplied to
  [`lm()`](https://rdrr.io/r/stats/lm.html) when performing the fit.

- backtrans:

  a function that transforms the response back to the original scale
  when the `formula` includes a transformation on `y`.
...
`predictdf`. Pointwise confidence or prediction bands are calculated
using the [`predict()`](https://rdrr.io/r/stats/predict.html) method.

## See also

[`lm()`](https://rdrr.io/r/stats/lm.html) for details on linear model
fitting.
