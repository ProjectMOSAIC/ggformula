...
`gf_rugx()` and `gf_rugy()` are versions that only add a rug to x- or y-
axis. By default, these functions do not inherit from the formula in the
original layer (because doing so would often result in rugs on both
axes), so the formula is required.
...
- gformula:
  ...
  A formula with shape `y ~ x` (`gf_rug()`) or `~ x` (`gf_rugx()`) or
  `~ y` (`gf_rugy()`).
