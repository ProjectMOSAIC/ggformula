...
engine: knitr
webr:
  packages: [ggformula, mosaic, mosaicData, NHANES, ggthemes, lubridate, palmerpenguins, tibble]
  cell-options:
    fig-width: 8
    fig-height: 3
---
...
```{webr}
#| setup: true
#| exercise: ["multiple-variable-quickhelp", "ribbon-to-linerange", "labs-demo", "labels", "scales", "scales2", "scales3", "brewer", "flip-violin", "flip-violin-orientation", "coord_equal", "coord_trans", "violinh", "horizontal", "filter", "gf_lims", "scale_lims", "gf_theme", "gf_theme_elements", "q_theme", "histogram-stat", "summary1", "summary3", "mean_se", "three-number"]
#| autorun: true
#| edit: false
#| output: false
library(tibble)
...
Edit the code below to reveal the formula shape for some of the following:
`gf_linerange()`,
`gf_errorbar()`,
`gf_errorbarh()`,
`gf_ribbon()`,
...
```{webr}
#| exercise: multiple-variable-quickhelp
gf_pointrange()
```
:::
...
gf_ribbon(
  ...
  fill = ~ city, alpha = 0.6)  |>
  gf_refine(
    scale_fill_manual(values = c("red", "blue", "orange", "navy", "gray50"))
  )
```
:::
...
#### `coord_flip()` often not needed.

Many `ggplot2` geoms determine whether to draw horizontal or vertical
version based on the types of data supplied in the variables, so `ccord_flip()`
can often be avoid by simply reversing which variable goes where.

```{webr}
#| exercise: flip-violin-orientation
gf_violin(city ~ high_temp, data = Weather)
```

If the default orientation isn't the desired one, the `orientation` argument
...
```{webr}
#| exercise: histogram-stat
gf_histogram( ~ high_temp | city, data = Weather)
gf_bar( ~ high_temp | city, data = Weather, stat = "bin")
```

This also explains why the message reported when you create a histogram without
 ...
 * `stat_count()` gets used with `gf_bar()` (`gf_col()` uses `stat_identity()`)
 ...
 Occasionally it is useful to use a non-default stat. 
 In particular, `stat_summary()` and `stat_summary_bin()` can be used 
 to avoid common data transformation situations where
 a function is used to aggregate over unique values of `x` or 
 over bins of `x` values.
...
These stats are designed to work with `gf_linerange()` and `gf_pointrange()`, 
so three summary functions can be specified.  
`fun.ymin` and `fun.ymax` return results that
are available in `after_stat(ymin)` and `after_stat(ymax)`.

```{webr}
#| exercise: summary3
# Plot min, median, and max high temperature each month
gf_pointrange(
  high_temp + ..ymin.. + ..ymax.. ~ (month(date, label = TRUE)), 
  color = ~ city, 
  data = Weather,
...
#| exercise: mean_se
gf_pointrange(
  high_temp + after_stat(ymin) + after_stat(ymax) ~ (month(date, label = TRUE)), color = ~ city,
  data = Weather, 
  stat = "summary", 
...
gf_pointrange(
  high_temp + after_stat(ymin) + afer_stat(ymax) ~ (month(date, label = TRUE)), color = ~ city,
